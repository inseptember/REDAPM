...
from model_mlm import BertForMultiLabelPrediction, BertForMaskedLM, MyBertConfig
...
def main():
    ...
    mlm_bert = BertForMaskedLM(config)
    mlm_bert = torch.load(args.model_name_or_path, map_location='cpu')
    model = BertForMultiLabelPrediction(config, 1)
    model.bert.load_state_dict(mlm_bert.bert.state_dict())
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                outputs = model(**batch)
            predict_probs = outputs[1].sigmoid()
