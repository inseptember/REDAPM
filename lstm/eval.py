...
def main():
    ...
    model = LstmForSequenceClassification.from_pretrained(args.model_name_or_path, config=config)
    ...
    def preprocess_function(examples):
        ...
        def merge_visit(texts, v_mask):
            m_texts = []
            m_mask = []
            for t, m in zip(texts, v_mask):
                m_texts.append(t[0])
                m_texts.append(t[1])
            return '. '.join(m_texts)
        ...
        for i, j in zip(examples['visit_sn'], examples['disease_names']):
            ...
            texts = merge_visit(texts, v_mask)
        ...
        drug_result = tokenizer(['[SEP]'.join(i.split(';')) for i in examples['drug_name']], padding=padding, max_length=args.max_length, truncation=True)
